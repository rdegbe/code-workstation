...
def withdraw_cash(money, overdraft, file, account, gone_overdraft):
   if overdraft == 'True':
...
def deposit(file, userMoney, account):
   ...
   while True:
       ...
       if money != 'done':
           all_deposits.append(money)
           time.sleep(2)
           print("Deposit accepted")
       else:
           break
   total = 0
   for i in range(len(all_deposits)):
       total += Decimal(all_deposits[i])
...
def overdraftfunction(howfar, email, time, file):
   ...
   # body of text
   body = "Dear Sir/Madam, Your recent transaction at (%s) shows that you have gone into your overdraft.  " \
          "Your overdraft protection is $%s.  " \
   ...
   return replace_file(file[0], file)
...
def main_menu(file):
   while True:
       ...
       for char in name:
           ...
           sys.stdout.flush()
           time.sleep(.05)
       print("\nWelcome to the menu. Select one of the following options:"
       ...
       if navigation == '1':
           ...
           file = withdraw_cash(Decimal(file[4]), file[3], file, file[0], file[5])
       elif navigation == '4':
